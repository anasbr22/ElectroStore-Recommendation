...
CREATE TABLE recommendations (
    ...
    FOREIGN KEY (product_id) REFERENCES tout_produit(id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE Action (
    ...
    type_action VARCHAR(255),             -- Type de l'action (achat, vue, etc.)
    produit_id INT,                       -- ID du produit concerné par l'action
    date_action DATETIME,                 -- Date et heure de l'action
    FOREIGN KEY (utilisateur_id) REFERENCES Utilisateur(id),  -- Clé étrangère vers la table Utilisateur
    FOREIGN KEY (produit_id) REFERENCES Produit(id)           -- Clé étrangère vers la table Produit
);
